...
const uploadeImage = async (base64EncodedImage) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      alert(
        'fail to upload, please choose image file with size less than 1MB!'
      );
    }
  } catch (err) {
    console.log(
      'fail to upload the page, please double check the size and make sure it is png or jpeg file'
    );
  }
};
