...
router.put('/:id', withAuth, async (req, res) => {
    try {
        ...
        if (!dbRatingData) {
            ...
            return;
        } else if (dbRatingData.get({plain: true}).poster_id !== req.session.userId) {
            res.status(400).json({ message: 'Sorry you cannot edit the rating with that id!' });
        } else {
...
router.delete('/:id', withAuth, async (req, res) => {
    try {
        ...
        if (!dbRatingData) {
            ...
            res.status(400).json({ message: 'Sorry you cannot delete the rating with that id!' });
        } else {
